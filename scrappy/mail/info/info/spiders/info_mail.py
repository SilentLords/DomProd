...
class InfoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if response.url.split('-')[-1].replace('/', '').replace('?osale2', '').replace('?osale1', '').__len__() > 10:
            h = int(response.url.split('-')[-1].replace('/', '').replace('?osale2', '').replace('?osale1', '')) % 10000000
        else:
            h = int(response.url.split('-')[-1].replace('/', '').replace('?osale2', '').replace('?osale1', ''))
